...
def get_color(tag1, tag2, gradient='RWG'):
    ...
    if gradient.upper()=='RYG':
        h=ratio/3  # Red - Yellow - Green
        l=0.45+abs(ratio-0.5)*2/5  # Luminosity, 0.5-1.0-0.5
        l=max(0.0,min(1.0,l))
        s=(abs(ratio-0.5)*2)**3
        col=colorsys.hls_to_rgb(h,l,1)
    elif gradient.upper()=='RWG' or True:  # Remove True if you add more gradients.
        if ratio<=0.5:
            h=0
        else: h=1/3  # Green
