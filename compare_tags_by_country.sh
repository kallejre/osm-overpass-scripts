...
asof=${asof:-""}
if [ ! -z "$asof" ]  # If asof is defined, then convert it to Overpass-compatible timestamp
then
  ql_date="[date:\"$(date -d "$asof" +"%FT%H:00:00Z")\"]"
  #asof="-$asof"  # Add prefix to asof, it will be used for csv output name
